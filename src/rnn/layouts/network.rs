...
#[derive(Debug)]
struct Modes {
    monitoring_mode: RwLock<MonitoringMode>,
}
...
#[derive(Debug)]
struct MonitoringChannel<S>
where
    S: Arithmetic,
{
    sender: Arc<mpsc::Sender<Status<S>>>,
    store: Arc<RwLock<Vec<Status<S>>>>,
}

pub enum NetworkContent<S>
where
    S: Arithmetic,
{
    Empty,
    NonEmpty(InputLayer<S>, OutputLayer<S>),
}
    ...
    S: Arithmetic,
{
    ...
    commands_channel: Arc<CommandsCh>,

    /// The monitoring channel stuff. Neurons are sending status messages to
    /// the network when enabled monitoring mode
    monitoring_channel: MonitoringChannel<S>,
...
where
    S: Arithmetic,
{
    pub fn new() -> Result<Network<S>, Box<dyn Error>> {
        let modes = Modes {
            monitoring_mode: RwLock::new(MonitoringMode::None),
        };

        let (monitoring_sender, monitoring_receiver) = mpsc::channel(CHANNEL_CAPACITY);
        let monitoring_channel = MonitoringChannel {
        ...
        let commands_channel = CommandsCh {
        ...
        .map(|id| Network {
            id,
            content: NetworkContent::Empty,
            modes: Arc::new(modes),
            commands_channel: Arc::new(commands_channel),
            monitoring_channel,
            receivers_tracker: TaskTracker::new(),
            cancel_token: CancellationToken::new(),
        })?;

        let monitoring_store_cloned = net.monitoring_channel.store.clone();
    ...
    async fn monitoring_save_task(
        monitoring_store: Arc<RwLock<Vec<Status<S>>>>,
        mut monitoring_receiver: mpsc::Receiver<Status<S>>,
    ) {
        while let Some(neuron_state) = monitoring_receiver.recv().await {
            let mut w_monitoring_store = monitoring_store.write().await;
            w_monitoring_store.push(neuron_state); // TODO Check when this store clear
        }
    }

    pub async fn set_monitoring_mode(&self, mode: MonitoringMode) {
        let mut w_monitoring_mode = self.modes.monitoring_mode.write().await;
        *w_monitoring_mode = mode.clone();
    ...
    pub async fn monitoring_mode(&self) -> MonitoringMode {
        self.modes.monitoring_mode.read().await.clone()
    }

    pub async fn get_commands_receiver(&self) -> broadcast::Receiver<NeuronCommand> {
        self.commands_channel.sender.subscribe()
    }

    pub fn monitoring_sender(&self) -> mpsc::WeakSender<Status<S>> {
        self.monitoring_channel.sender.downgrade()
    }
}

// /// Network is a high level container to other containers (neurons)
// #[derive(Debug)]
// pub struct OldNetwork<S>
// where
//     S: Arithmetic,
// {
//     /// The network id
...
//     /// Inner neurons
//     neurons: RwLock<BTreeMap<String, Arc<Neuron<S>>>>,

//     /// Network's modes set
//     modes: Arc<RwLock<Modes>>,

//     /// Network's input ports
//     input_interface: Arc<RwLock<BTreeMap<usize, Arc<RwLock<PortCore<S>>>>>>,

//     /// Network's output ports
//     output_interface: Arc<RwLock<BTreeMap<usize, Arc<RwLock<PortCore<S>>>>>>,
...
//     pub async fn get_neuron(&self, id: &str) -> Option<Arc<Neuron<S>>> {
...
//         bias: S,
//         input_configs: Vec<InputCfg<S>>,
//     ) -> Result<Arc<Neuron<S>>, Box<dyn std::error::Error>> {
...
//     pub async fn input(&self, signal: Signal<S>, port: usize) -> Result<usize, Box<dyn Error>> {
...
//     pub async fn pop_monitoring_store(&self) -> Vec<Status<S>> {
...
//         &self,
//         port: usize,
//     ) -> Option<Arc<RwLock<Receiver<Signal<S>>>>> {
...
//             recent_signal_value: match recent_signal.value() {
//                 SignalType::Spike => S::one(),
...
// impl<S> fmt::Display for Network<S>
// where
//     S: Arithmetic,
// {
...
//         let n1 = Network::<i16>::new().unwrap();
//         let n2 = Network::<i16>::new().unwrap();
...
//         let net_orig = Arc::new(Network::<i16>::new().unwrap());
...
//         let net = Arc::new(Network::<i16>::new().unwrap());
...
//         let net = Network::<i16>::new().unwrap();
...
//         assert!(
//             net.input(Signal::<i16>::new(SignalType::Spike), 0)
//                 .await
//                 .is_ok()
//         );
